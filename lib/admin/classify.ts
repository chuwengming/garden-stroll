...
export type AdminQueryKind = "total" | "list" | "top_customers";
export type AdminQueryRange = "month" | "last_month" | "today";
  ...
  if (aiApiKey()) {
    try {
      const prompt = [
        ...
        "",
        "判斷原則：",
        "- 查庫請求 = 想看預約總量、預約列表、常客排名、營業統計等**管理資料**。",
        ...
        "- today：今天／今日",
        "",
        "只輸出以下 JSON（只輸出 json，不要其它文字）：{\"isAdminQuery\":true|false,\"kind\":\"total\"|\"list\"|\"top_customers\",\"range\":\"month\"|\"last_month\"|\"today\"}",
        `管理員訊息：${text}`,
      ].join("\n");
      ...
      if (
        ...
        (parsed.isAdminQuery === false ||
          (["total", "list", "top_customers"].includes(parsed.kind) &&
            ["month", "last_month", "today"].includes(parsed.range)))
      ) {
        return {
